...
%   Area parameters and number of iterations are global variables
    ...
    %   Simulation parameters
        ...
        t_f_vf=20*60;               %   Final time for the Vector field part. Note it is not its duration but the time at which it stops
    ...
    %   Geolocation process parameters
        ...
        if strcmp(filter,'UKF_aug') || strcmp(filter,'EKF_aug')
            Gratio_cov = 1.8593;                                                                % Estimated Gain Ratio Covariance
            Gratio_cov = 0.001;                                                                % Estimated Gain Ratio Covariance
...
%             R_KF = 2.8^2                                                                          % Measurement Noise
        ...
        elseif strcmp(filter, 'PF_aug')
            ...
            F_KF = eye(3);                                                                      % Dynamics matrix: unity because model is static
            G_KF = eye(1);                                                                        % Noise matrix: unity for pure additive gaussian noise
            Q_KF = diag([10, 10, 0.00001])                                                       % Process Noise
            R_KF = sqrt(50)^2                                                                          % Measurement Noise
            x_state_ini = [x_bnd/2 y_bnd/2]'                                                    % Initail State Guess - Middle of the area and middle Gain Ratio
            Gt_ratio_ini = 1;
            P_cov_ini=diag([(x_bnd/2)^(2)/12 (x_bnd/2)^(2)/12 Gratio_cov])  % Initial State Covariance
        elseif strcmp(filter, 'PF')
            ...
            F_KF = eye(2);                                                                      % Dynamics matrix: unity because model is static
            G_KF=eye(1);                                                                        % Noise matrix: unity for pure additive gaussian noise
            Q_KF = diag([2, 2])                                                                 % Process Noise
            R_KF = 10^2                                                                   % Measurement Noise
            x_state_ini = [x_bnd/2 y_bnd/2]'                                                    % Initail State Guess - Middle of the area and middle Gain Ratio
            Gt_ratio_ini = 1;
            P_cov_ini = diag([(x_bnd/2)^(2)/12 (x_bnd/2)^(2)/12])                               % Initial State Covariance
        end
...
%         F_filter = @(x_uav_0, x_uav_k, alpha, x_jam_old, P_cov_old)EKFilter(x_uav_0, x_uav_k, alpha, x_jam_old, P_cov_old, F_KF, G_KF, Q_KF, R_KF, N_grid);
    ...
    case('smooth')
        ...
        system.x_uav = nan(3,N_loops_vf);
        system.P_cov_state = nan(2,2,N_loops_vf);  
...
%     sim('look_up_aid_w_ducati');                                                                          	%   Jammer gain interpolation
    ...
    %   Filtering
        ...
        system.alpha(1:k,1) = P_r_filt_ratio(1:k,1);
        system.step = k;
...
mean_exec_time = nanmean(exec_time)
if exist('test_running','var') && test_running
    ...
    if strcmp(filter, 'EKF') || strcmp(filter, 'UKF') || strcmp(filter, 'smooth')
        ...
        axis([0, N_loops_vf*D_T, 3000, 9000]);
    ...
    elseif strcmp(filter, 'EKF_aug') || strcmp(filter, 'UKF_aug')
        figure(1)
        hold on;
        grid on;
        grid minor;
