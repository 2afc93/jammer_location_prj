...
%   Area parameters and number of iterations are global variables
    ...
    %   Geolocation process parameters
        ...
        %   EKF (UKF) initialisation:
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if strcmp(filter,'PF')
                Q_KF = 10*eye(2)                                            %   Process noise matrix: better to be small std for position and power
            else
                Q_KF = 0.1*eye(2)                                          
            end
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            uni_range = 6000;                                               % Uniform Distribution Range
            P_cov_ini = uni_range^2/12*diag([1 1])                         % Uniform Distribution Covariance is given by A^2/12 (A is the range)
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
system.alpha = zeros(N_loops_vf,1);
...
%             x_state(:,k) = x_t_vec;
            ...
            system = F_filter(system);
            exec_time(k) = toc; 
%             [x_state(:,k), P_cov(:,:,k), K_EKF_gain(:,k)] = F_filter([x_vec_all(1,:), h_0]', [x_vec_all(k,:), h_0]', P_r_filt_ratio(k,1), x_state_ini, P_cov_ini);
            x_state(:,k) = system.x_jam(:,k);
            re_run_bool=0;
          	div_EKF_bool=0;
        ...
        elseif (re_run_bool==0)                                             %   Normal operation condition: the EKF (UKF) has converged and remains on target
            ...
            % Filtering
%             [x_state(:,k), P_cov(:,:,k), K_EKF_gain(:,k)] = F_filter([x_vec_all(1,:), h_0]', [x_vec_all(k,:), h_0]', P_r_filt_ratio(k,1), x_state(:,k-1), P_cov(:,:,k-1));
            tic
            system = F_filter(system);
            exec_time(k) = toc; 
            x_state(:,k) = system.x_jam(:,k);
...
%     plot([(N_loops_fb+1):N_loops_vf]*D_T, system.x_jam(1,(N_loops_fb+1):N_loops_vf), 'b', 'Linewidth', 2.0);
    ...
    ylabel('Position (metres)', 'Fontsize', 14, 'Fontweight', 'bold')
...
%         plot([(N_loops_fb+1):N_loops_vf]*D_T, system.K_gain(1,(N_loops_fb+1):N_loops_vf), 'b', 'Linewidth', 2.0);
        xlabel('Time [seconds]', 'Fontsize', 14, 'Fontweight', 'bold')
        ylabel('Kalman Gain [metres]', 'Fontsize', 14, 'Fontweight', 'bold')
