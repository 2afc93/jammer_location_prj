function [ system_post ] = UK_EK_Smoother(system_prio, F, G, Q, R, N_back, k_final)
    ...
    t_start = 200;               % This is the simulation step at which the filtering starts
    t_end   = system_prio.step;
    N_back  = min(N_back, k_final - t_end); 
    
    %% Forward Filtering: UKF estimation at simulation step  
    % The standard UKF filter is employed to get the estimation at the
    % simulation time
    
    system_post = UKFilter_augmented(system_prio, F, G, Q, R);
    t_smooth = t_end - N_back;
    if t_smooth > t_start
        [y_t_prio, S_t_prio] = smooth(system_post, t_smooth, F, Q, R);
        
        P_back_t = eye(size(S_t_prio))/S_t_prio;
        x_back_t = P_back_t*y_t_prio;
        
        x_for_t = [system_post.x_jam(:,t_smooth); system_post.Gt_ratio(t_smooth)];
        ...
        system_post.x_jam(:,t_smooth) = x_s_t(1:2);
        system_post.Gt_ratio(t_smooth) = x_s_t(3);
        system_post.P_cov_state(:,:,t_smooth) = P_s_t;
%         system_post.K_gain(:,t_smooth) = K_t;
    end
...
function [y_t_prio, S_t_prio] = smooth(system, t, F, Q, R)
    if system.step == t
        ...
        H_tp1 = JH(system.x_uav(:,1), system.x_uav(:,t), system.x_jam(:,t), system.Gt_ratio(:,t));
...
function H = JH(x_uav_0, x_uav_k, x_jam_k, Gt_ratio)
    ...
    H(1,3) = ( (x_jam_k(1,:)-x_uav_0(1)).^2 + (x_jam_k(2,:)-x_uav_0(2)).^2 + x_uav_0(3).^2 ) ./( (x_jam_k(1,:)-x_uav_k(1)).^2 + (x_jam_k(2,:)-x_uav_k(2)).^2 + x_uav_k(3).^2 );
end
