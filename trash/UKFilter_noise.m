...
function [ system_posteriori ] = UKFilter_noise(system_priori, F_KF, G_KF, Q_KF, R_KF)
    %% Data Acquisition
    ...
    if k > 10000
        R_KF = 0.07; 
        alpha = system_priori.alpha(k)/system_priori.alpha(k-10);
        x_uav_0 = system_priori.x_uav(:,k-10);
    end
    x_uav_k             = system_priori.x_uav(:,k);
    P_cov_old           = system_priori.P_cov_state(:,:,k-1);
    system_posteriori   = system_priori;
       
    hmeas               = @(x_jam)h_alpha(x_jam, x_uav_0, x_uav_k);     % Measurement function
    
    
    %% Prediction (Linear Process)
    % State Propagation
    x_pred = F_KF*x_jam_old;
    ...
    alpha_param         = 1.8;                              % default, tunable
    ki                  = 1;                                % default, tunable
    beta                = 2;                                % default, tunable
    lambda              = alpha_param^2*(L+ki)-L;           % scaling factor
    c                   = L+lambda;                         % scaling factor
    Wm                  = [lambda/c 0.5/c+zeros(1,2*(L+1))];% Mean Weights
    Wc                  = Wm;                               % Covariance Weights
    Wc(1)               = Wc(1)+(1-alpha_param^2+beta);     % weights for covariance
    c                   = sqrt(c);                          %
    
    x_aug = [x_pred;0];
    P_aug = blkdiag(P_pred, 10);
    X_sigmas = sigmas(x_aug, P_aug, c);
    [z_mean,Z_t,S,Pz] = ut(hmeas, X_sigmas, Wm, Wc, m);   % Measurement Unscented Transformation
    Pxz = zeros(size(x_aug));
    
    for jj = 1:2*length(x_aug)+1
        Pxz = Pxz + Wc(jj)*(X_sigmas(:, jj) - x_aug)*(Z_t(jj) - z_mean)';
    end
    
    % Innovation
    v = alpha - z_mean;                 
    K_gain = Pxz/S;                                         % Kalman Gain
    x_state = x_aug + K_gain*v;                            % State update
    P_cov_new = P_aug - K_gain*S*K_gain';                  % Covariance Update
   
    system_posteriori.P_cov_state(:,:,k) = P_cov_new(1:2,1:2);
    system_posteriori.x_jam(:,k) = x_state(1:2);
    system_posteriori.K_gain(:,k) = K_gain(1:2);
end
...
function [y,Y,P,Y1] = ut(h, X, Wm, Wc, n)
...
function h = h_alpha_ext(x_jam_k, x_uav_0, x_uav_k, Gt_ratio)
    h = Gt_ratio.*(( (x_jam_k(1,:)-x_uav_0(1)).^2 + (x_jam_k(2,:)-x_uav_0(2)).^2 + x_uav_0(3).^2 ) ./( (x_jam_k(1,:)-x_uav_k(1)).^2 + (x_jam_k(2,:)-x_uav_k(2)).^2 + x_uav_k(3).^2 ));
end