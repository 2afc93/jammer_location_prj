...
%   Area parameters and number of iterations are global variables
    ...
    %   Simulation parameters
        ...
        t_f_vf=30*60;               %   Final time for the Vector field part. Note it is not its duration but the time at which it stops
    ...
    %   Geolocation process parameters
        ...
        if strcmp(filter,'UKF_aug') || strcmp(filter,'EKF_aug') || strcmp(filter,'IEKF_aug')
            Gratio_cov = 1.8593;                                                                % Estimated Gain Ratio Covariance
            Gratio_cov = 0.0001;                                                                % Estimated Gain Ratio Covariance
            minG_ratio = 0.0149; maxG_ratio = 66.9633;                                          % Minimum/Maximum transmitter gain ratio
            F_KF = eye(3);                                                                      % Dynamics matrix: unity because model is static
            G_KF = eye(1);                                                                        % Noise matrix: unity for pure additive gaussian noise
            Q_KF = diag([2, 2, 0.007])                                                            % Process Noise
            R_KF = 2.85^2                                                                          % Measurement Noise
            x_state_ini = [x_bnd/2 y_bnd/2]'                                                    % Initail State Guess - Middle of the area and middle Gain Ratio
            Gt_ratio_ini = 1;
            P_cov_ini=diag([(x_bnd/2)^(2)/12 (x_bnd/2)^(2)/12 Gratio_cov])                        % Initial State Covariance
        elseif strcmp(filter, 'PF_aug')
            Gratio_cov = 1.8593;                                                                % Estimated Gain Ratio Covariance
            minG_ratio = 0.0149; maxG_ratio = 66.9633;                                          % Minimum/Maximum transmitter gain ratio
            F_KF = eye(3);                                                                      % Dynamics matrix: unity because model is static
            G_KF = eye(1);                                                                        % Noise matrix: unity for pure additive gaussian noise
            Q_KF = diag([10, 10, 0.0001])                                                       % Process Noise
            R_KF = sqrt(5)^2                                                                          % Measurement Noise
            x_state_ini = [x_bnd/2 y_bnd/2]'                                                    % Initail State Guess - Middle of the area and middle Gain Ratio
            Gt_ratio_ini = 1;
            P_cov_ini=diag([(x_bnd/2)^(2)/12 (x_bnd/2)^(2)/12 Gratio_cov])  % Initial State Covariance
        elseif strcmp(filter, 'PF')
            F_KF = eye(2);                                                                      % Dynamics matrix: unity because model is static
            G_KF = eye(1);                                                                        % Noise matrix: unity for pure additive gaussian noise
            Q_KF = 10*eye(2)                                                                   % Process Noise
            R_KF = sqrt(100)^2                                                                          % Measurement Noise
            x_state_ini = [x_bnd/2 y_bnd/2]';                                                   % Initail State Guess - Middle of the area and middle Gain Ratio
            Gt_ratio_ini = 1;
            P_cov_ini=diag([(x_bnd/2)^(2)/12 (x_bnd/2)^(2)/12])                                 % Initial State Covariance
        elseif strcmp(filter, 'smooth')
            Gratio_cov = 1.8593;                                                                % Estimated Gain Ratio Covariance
            Gratio_cov = 0.0001;                                                                % Estimated Gain Ratio Covariance
            minG_ratio = 0.0149; maxG_ratio = 66.9633;                                          % Minimum/Maximum transmitter gain ratio
            F_KF = eye(3);                                                                      % Dynamics matrix: unity because model is static
            G_KF = eye(1);                                                                        % Noise matrix: unity for pure additive gaussian noise
            Q_KF = diag([2, 2, 0.005])                                                            % Process Noise
            R_KF = 2.8^2                                                                          % Measurement Noise
            x_state_ini = [x_bnd/2 y_bnd/2]'                                                    % Initail State Guess - Middle of the area and middle Gain Ratio
            Gt_ratio_ini = 1;
            P_cov_ini=diag([(x_bnd/2)^(2)/12 (x_bnd/2)^(2)/12 Gratio_cov])                        % Initial State Covariance
        else
        ...
        % Parameters for the filter rerun
        re_run_bool=0;     	 
...
system.x_uav = zeros(3,N_loops_fb);
...
G_t_sto = nan(N_loops_vf,1);
Gt_ratio = nan(N_loops_vf,1);
...
system.alpha = zeros(N_loops_fb, 1);
...
switch filter
    case('UKF')
%         F_filter = @(x_uav_0, x_uav_k, alpha, x_jam_old, P_cov_old)UKFilter(x_uav_0, x_uav_k, alpha, x_jam_old, P_cov_old, F_KF, G_KF, Q_KF, R_KF);
        system.x_jam = zeros(2,N_loops_vf);
        system.x_uav = zeros(3,N_loops_vf);
        system.P_cov_state = zeros(2,2,N_loops_vf);  
        system.Gt_ratio     = zeros(1,N_loops_vf);
        system.K_gain = zeros(2,N_loops_vf);
        F_filter = @(system_data)UKFilter(system_data, F_KF, G_KF, Q_KF, R_KF);
    case('EKF')
%         F_filter = @(x_uav_0, x_uav_k, alpha, x_jam_old, P_cov_old)EKFilter(x_uav_0, x_uav_k, alpha, x_jam_old, P_cov_old, F_KF, G_KF, Q_KF, R_KF);
        system.x_jam = zeros(2,N_loops_vf);
        system.x_uav = zeros(3,N_loops_vf);
        system.P_cov_state = zeros(2,2,N_loops_vf);
        system.Gt_ratio     = zeros(1,N_loops_vf);
        system.K_gain = zeros(2,N_loops_vf);
        F_filter = @(system_data)EKFilter_new(system_data, F_KF, G_KF, Q_KF, R_KF);
    case('PF')
%         F_filter = @(x_uav_0, x_uav_k, alpha, x_jam_old, P_cov_old)EKFilter(x_uav_0, x_uav_k, alpha, x_jam_old, P_cov_old, F_KF, G_KF, Q_KF, R_KF, N_grid);
        N_part = 150;
        system.particles.x  = nan(N_loops_vf,N_part);
        system.particles.y  = nan(N_loops_vf,N_part);
        system.w            = nan(N_part,N_loops_vf);
        system.P_cov_state  = zeros(2,2,N_loops_vf);
        system.x_jam        = zeros(2,N_loops_vf);
        system.Gt_ratio     = zeros(1,N_loops_vf);
        system.K_gain       = nan(2,N_loops_vf);
        F_filter = @(system_data)PFilter(system_data, F_KF, G_KF, Q_KF, R_KF, N_part);
    case('PF_aug')
        N_part = 200;
        system.particles.x  = nan(N_loops_vf,N_part);
        system.particles.y  = nan(N_loops_vf,N_part);
        system.particles.G  = nan(N_loops_vf,N_part);
        system.w.xy         = nan(N_part,N_loops_vf);
        system.w.G          = nan(N_part,N_loops_vf);
        system.P_cov_state  = zeros(3,3,N_loops_vf);
        system.x_uav        = zeros(3,N_loops_vf);
        system.x_jam        = zeros(2,N_loops_vf);
        system.Gt_ratio     = zeros(1,N_loops_vf);
        system.K_gain       = nan(3,N_loops_vf);
        F_filter = @(system_data)PFilter_augmented(system_data, F_KF, G_KF, Q_KF, R_KF, N_part);
    case('UKF_aug')
        system.x_jam = zeros(2,N_loops_vf);
        system.x_uav = nan(3,N_loops_vf);
        system.v_uav = nan(3,N_loops_vf);
        system.P_cov_state = zeros(3,3,N_loops_vf);
        system.Gt_ratio     = zeros(1,N_loops_vf);
        system.K_gain = zeros(3,N_loops_vf);
        F_filter = @(system_data)UKFilter_augmented(system_data, F_KF, G_KF, Q_KF, R_KF);
    case('EKF_aug')
        system.x_jam = zeros(2,N_loops_vf);
        system.x_uav = nan(3,N_loops_vf);
        system.v_uav = nan(3,N_loops_vf);
        ...
        system.Gt_ratio     = zeros(1,N_loops_vf);
        system.K_gain = zeros(3,N_loops_vf);
        F_filter = @(system_data)EKFilter_augmented(system_data, F_KF, G_KF, Q_KF, R_KF);
    case('smooth')
        N_back = 50;
        ystem.x_jam = nan(2,N_loops_vf);
        system.x_uav = nan(3,N_loops_vf);
        system.P_cov_state = nan(3,3,N_loops_vf);  
        system.Gt_ratio     = nan(1,N_loops_vf);
        system.K_gain = nan(3,N_loops_vf);
        F_filter = @(system_data)UK_EK_Smoother(system_data, F_KF, G_KF, Q_KF, R_KF, N_back, N_loops_vf);
    otherwise
        error('The indicated filter does not match any of the available ones: EKF, UKF or PF. Check the spelling.') 
end
...
%     sim('look_up_aid_w_ducati');                                                                          	%   Jammer gain interpolation
    G_t_sim = interp2(azimvals,elevvals,gainvals',azimuth_rel_angle(k,1)*r2d,elev_angle(k,1)*r2d);
    G_t=G_t_sim;
    G_t_sto(k,1) = G_t;
    ...
    if (k>3*butter_order+1)                                                                         %   If simulation has initialised
       ...
       Gt_ratio(k,1) = G_t_sto(k,1)/G_t_sto(1,1);                                                   %   Transmitter Gain Ratio
    ...
    %   Filtering
        ...
        system.x_uav(:,k) = [x_vec_all(k,:), h_0]';
        system.v_uav(:,k) = [x_vec_dot(k,:), 0]';
        system.alpha(k,1) = P_r_filt_ratio(k,1);
        system.step = k;
...
%             x_state(:,k) = x_t_vec;
            ...
            system = F_filter(system);
            if strcmp(filter,'smooth')
                k_smooth = max(1,k-N_back);
                x_state(:,k) = system.x_jam(1:2,k_smooth);
            else
                x_state(:,k) = system.x_jam(1:2,k);
        ...
        elseif ((obs_check==1)&&(k>k_obs))                                  %   If intersections have begun & filter has alreay started
            ...
            system.x_jam(:,k) = x_state_ini;
            system.Gt_ratio(:,k) = Gt_ratio_ini;
            x_state(:,k) = system.x_jam(1:2,k);
            system.P_cov_state(:,:,k) = P_cov_ini;
        end
...
% system.x_uav = [system.x_uav, zeros(3, N_loops_vf-N_loops_fb)];
...
system.alpha = [system.alpha; zeros(N_loops_vf-N_loops_fb,1)];
...
%     system.x_jam = [system.x_jam, zeros(2,N_loops_vf-N_loops_fb)];
    P_cov(:,:,N_loops_fb+1:N_loops_vf)=0;                                   %   filter Covariance matrix for all
%     system.P_cov_state(:,:,N_loops_fb+1:N_loops_vf) = 0;
%     K_EKF_gain=[K_EKF_gain zeros(2,N_loops_vf-N_loops_fb)];              	%   Kalman gain storage
%     system.K_gain = [system.K_gain, zeros(2,N_loops_vf-N_loops_fb)];
...
%             x_state(:,k) = x_t_vec;
            ...
            system.x_jam(:,k-1) = x_state_ini;
            system.Gt_ratio(:,k-1) = Gt_ratio_ini;
            system.P_cov_state(:,:,k-1) = P_cov_ini;
            system = F_filter(system);
            %%%%% [x_state(:,k),P_cov(:,:,k),K_EKF_gain(:,k)]=KF_form(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);
            if strcmp(filter,'smooth')
                k_smooth = max(1,k-N_back);
...
if exist('test_running','var') && test_running
    ...
    if strcmp(filter, 'EKF') || strcmp(filter, 'UKF') || strcmp(filter, 'smooth')
        figure(1)
        hold on;
        grid on;
        grid minor;
        plot([1:N_loops_vf]'*D_T, x_t_vec(1)*ones(N_loops_vf,1), '--r', 'Linewidth', 1.5);
        plot([1:N_loops_vf]'*D_T, x_t_vec(2)*ones(N_loops_vf,1), '--b', 'Linewidth', 1.5);
        plot([1:N_loops_vf]*D_T, system.x_jam(1,:), 'r', 'Linewidth', 2.0);
        plot([1:N_loops_vf]*D_T, system.x_jam(2,:), 'b', 'Linewidth', 2.0);
        xlabel('Time [sec]', 'Fontsize', 14, 'Fontweight', 'bold')
        ylabel('Position [m]', 'Fontsize', 14, 'Fontweight', 'bold')
        axis([0, N_loops_vf*D_T, 0.9*min(min(system.x_jam)), 1.1*max(max(system.x_jam))]);
        
        figure(2)
        hold on;
        grid on;
        grid minor;
        plot([1:N_loops_vf]*D_T, system.K_gain(1,:), 'r', 'Linewidth', 2.0);
        plot([1:N_loops_vf]*D_T, system.K_gain(2,:), 'b', 'Linewidth', 2.0);   
        xlabel('Time [sec]', 'Fontsize', 14, 'Fontweight', 'bold')
        ylabel('Kalman Gain [m]', 'Fontsize', 14, 'Fontweight', 'bold')
        axis([0, N_loops_vf*D_T, 1.1*min(min(system.K_gain)), 1.1*max(max(system.K_gain))]);
    
    elseif strcmp(filter, 'EKF_aug') || strcmp(filter, 'UKF_aug') || strcmp(filter, 'IEKF_aug')
        figure(1)
        hold on;
        grid on;
        grid minor;
        ...
        grid on;
        grid minor;
        plot([1:N_loops_vf]'*D_T, Gt_ratio, '--r', 'Linewidth', 1.5);
        plot([1:N_loops_vf]*D_T, system.Gt_ratio, 'r', 'Linewidth', 2.0);
        xlabel('Time [sec]', 'Fontsize', 14, 'Fontweight', 'bold')
        ylabel('Transmitter Gain Ratio []', 'Fontsize', 14, 'Fontweight', 'bold')
        axis([0, N_loops_vf*D_T, 0.9*min(system.Gt_ratio), 1.1*max(system.Gt_ratio)]);
        ...
        grid on;
        grid minor;
        plot([1:N_loops_vf]*D_T, system.K_gain(3,:), 'r', 'Linewidth', 2.0);
        xlabel('Time [sec]', 'Fontsize', 14, 'Fontweight', 'bold')
        ylabel('Kalman Gain []', 'Fontsize', 14, 'Fontweight', 'bold')
        axis([0, N_loops_vf*D_T, 1.1*min(system.K_gain(3,:)), 1.1*max(system.K_gain(3,:))]);
    ...
    elseif strcmp(filter,'PF')
        ...
        grid on;
        grid minor;
        plot([1:N_loops_vf]'*D_T, squeeze(system.particles.x), 'r', 'Linewidth', 0.5);
        plot([1:N_loops_vf]'*D_T, squeeze(system.particles.y), 'b', 'Linewidth', 0.5);
        xlabel('Time [sec]', 'Fontsize', 14, 'Fontweight', 'bold')
        ylabel('Position Particles paths [m]', 'Fontsize', 14, 'Fontweight', 'bold')
        axis([0, N_loops_vf*D_T, 0.9*x_bnd/4, 1.1*3*x_bnd/4]);
        ...
        xlabel('Time [sec]', 'Fontsize', 14, 'Fontweight', 'bold')
        ylabel('Position [m]', 'Fontsize', 14, 'Fontweight', 'bold')
        axis([0, N_loops_vf*D_T, 2500, 9500]);
        ...
        ylabel('Particles paths X [metres]', 'Fontsize', 14, 'Fontweight', 'bold')
        axis([0, N_loops_vf*D_T, 0.9*x_bnd/4, 1.1*3*x_bnd/4]);
        ...
        grid on;
        grid minor;
        plot([1:N_loops_vf]'*D_T, squeeze(system.particles.G), 'b', 'Linewidth', 0.5)
        plot([1:N_loops_vf]'*D_T, Gt_ratio, 'r', 'Linewidth', 1.5);
        xlabel('Time [seconds]', 'Fontsize', 14, 'Fontweight', 'bold')
        ylabel('Transmitter Gain Ratio [-]', 'Fontsize', 14, 'Fontweight', 'bold')
...
% grid on;
% grid minor;
% plot([1:N_loops_vf]'*D_T, x_t_vec(1)*ones(N_loops_vf,1), 'r', 'Linewidth', 2.0);
% plot([1:N_loops_fb]*D_T, system.x_jam(1,1:N_loops_fb), '--b', 'Linewidth', 2.0);
% plot([(N_loops_fb+1):N_loops_vf]*D_T, system.x_jam(1,(N_loops_fb+1):N_loops_vf), 'b', 'Linewidth', 2.0);
% xlabel('Time (seconds)', 'Fontsize', 14, 'Fontweight', 'bold')
% ylabel('Position X (metres)', 'Fontsize', 14, 'Fontweight', 'bold')
% axis([0, N_loops_vf*D_T, 0, x_bnd]);
% if ~strcmp(filter,'PF')
%     figure(3)
%     hold on;
%     grid on;
%     grid minor;
%     plot([1:N_loops_fb]*D_T, system.K_gain(1,1:N_loops_fb), '--b', 'Linewidth', 2.0);
%     plot([(N_loops_fb+1):N_loops_vf]*D_T, system.K_gain(1,(N_loops_fb+1):N_loops_vf), 'b', 'Linewidth', 2.0);
%     xlabel('Time [seconds]', 'Fontsize', 14, 'Fontweight', 'bold')
%     ylabel('Kalman Gain X [metres]', 'Fontsize', 14, 'Fontweight', 'bold')
% else
%     figure(3)
%     hold on;
%     grid on;
%     grid minor;
%     plot([1:N_loops_fb]'*D_T, squeeze(system.particles.x(1:N_loops_fb,:)), 'y', 'Linewidth', 0.5);
%     plot([(N_loops_fb+1):N_loops_vf]'*D_T, squeeze(system.particles.x((N_loops_fb+1):N_loops_vf,:)), 'y', 'Linewidth', 0.5);
%     xlabel('Time [seconds]', 'Fontsize', 14, 'Fontweight', 'bold')
%     ylabel('Particles paths X [metres]', 'Fontsize', 14, 'Fontweight', 'bold')
% 
% end
...
% grid on;
% grid minor;
% plot([1:N_loops_vf]'*D_T, x_t_vec(2)*ones(N_loops_vf,1), 'r', 'Linewidth', 2.0);
% plot([1:N_loops_fb]*D_T, system.x_jam(2,1:N_loops_fb), '--b', 'Linewidth', 2.0);
% plot([(N_loops_fb+1):N_loops_vf]*D_T, system.x_jam(2,(N_loops_fb+1):N_loops_vf), 'b', 'Linewidth', 2.0);
% axis([0, N_loops_vf*D_T, 0, y_bnd]);
% xlabel('Time [seconds]', 'Fontsize', 14, 'Fontweight', 'bold')
% ylabel('Position Y [metres]', 'Fontsize', 14, 'Fontweight', 'bold')
% 
% if ~strcmp(filter,'PF')
%     figure(5)
%     hold on;
%     grid on;
%     grid minor;
%     plot([1:N_loops_fb]*D_T, system.K_gain(2,1:N_loops_fb), '--b', 'Linewidth', 2.0);
%     plot([(N_loops_fb+1):N_loops_vf]*D_T, system.K_gain(2,(N_loops_fb+1):N_loops_vf), 'b', 'Linewidth', 2.0);
%     xlabel('Time [seconds]', 'Fontsize', 14, 'Fontweight', 'bold')
%     ylabel('Kalman Gain Y [metres]', 'Fontsize', 14, 'Fontweight', 'bold')
% else
%     figure(5)
%     hold on;
%     grid on;
%     grid minor;
%     plot([1:N_loops_fb]'*D_T, squeeze(system.particles.y(1:N_loops_fb,:)), 'y', 'Linewidth', 0.5);
%     plot([(N_loops_fb+1):N_loops_vf]'*D_T, squeeze(system.particles.y((N_loops_fb+1):N_loops_vf,:)), 'y', 'Linewidth', 0.5);
%     xlabel('Time [seconds]', 'Fontsize', 14, 'Fontweight', 'bold')
%     ylabel('Particles paths Y [metres]', 'Fontsize', 14, 'Fontweight', 'bold')
% end
