...
if exist('test_running','var') && test_running
    filter = test_filter{test_iter_filter};
else
    filter = 'PF';
end
...
%     K_EKF_gain=zeros(2,N_loops_fb);                                           	%   Kalman gain storage
    ...
    system.x_jam = zeros(2,N_loops_fb);
    system.P_cov_state = zeros(2,2,N_loops_fb);
...
%         F_filter = @(x_uav_0, x_uav_k, alpha, x_jam_old, P_cov_old)UKFilter(x_uav_0, x_uav_k, alpha, x_jam_old, P_cov_old, F_KF, G_KF, Q_KF, R_KF);
        system.K_gain = zeros(2,N_loops_fb);
        F_filter = @(system_data)UKFilter_new(system_data, F_KF, G_KF, Q_KF, R_KF);
    case('EKF')
